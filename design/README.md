# First Step Montessori design concepts

Status: Approved direction
Owner: Project team
Last updated: 2026-08-30

The PNG boards in this folder are compact design showcases, not literal page-density specifications. The implemented site uses the same visual language with substantially more whitespace and responsive section spacing.

## Approved directions

- [Calm Prepared Garden concept](calm-prepared-garden-concept.png)
- [Joyful Geometry concept](joyful-geometry-concept.png)
- [GitHub organization avatar source](github-organization-avatar.svg)
- [GitHub organization avatar PNG](github-organization-avatar.png)
- [Calm Prepared Garden interactive canvas](https://p.superdesign.dev/draft/627d3fe1-1ab5-4eba-a4a1-f8a5673a3f1f)
- [Joyful Geometry interactive canvas](https://p.superdesign.dev/draft/865f4ff2-2ca9-4bdb-ae6a-d0f23027db65)

These are design references only. Generated placeholder copy, contact details, dates and programme ages are not authoritative. Website content must come from the verified content model and [`docs/18-content-verification-checklist.md`](../docs/18-content-verification-checklist.md).
